...
describe('FingerBoard', () => {
  describe('calculateFingerJointDimensions', () => {
    it('should calculate basic dimensions correctly', () => {
      ...
      expect(result.fingerWidth).toBeCloseTo(100 / expectedTotalSegmentsX / 100); // ≈ 0.0909
      expect(result.fingerHeight).toBeCloseTo(80 / expectedTotalSegmentsY / 100); // ≈ 0.0889
    });
  ...
  describe('generateHorizontalFingerPoints', () => {
    ...
    it('should generate correct number of finger segments', () => {
      ...
      const expectedMinPoints = 1 + baseDimensions.countX * 3 + (baseDimensions.countX - 1) + 1;
  ...
  describe('generateVerticalFingerPoints', () => {
    ...
    it('should generate correct number of finger segments', () => {
      ...
      const expectedMinPoints = 1 + baseDimensions.countY * 3 + (baseDimensions.countY - 1) + 1;
      expect(points.length).toBeGreaterThanOrEqual(expectedMinPoints - 5); // Allow some tolerance for implementation details
    });
  });
  ...
  describe('Integration tests', () => {
    it('should generate consistent points for a complete box', () => {
      ...
      const bottomPoints = generateHorizontalFingerPoints(dimensions, false, false, true);
      const rightPoints = generateVerticalFingerPoints(dimensions, true, false);
      const topPoints = generateHorizontalFingerPoints(dimensions, false, true, false);
    ...
    it('should handle zero thickness gracefully', () => {
      ...
      const points = generateHorizontalFingerPoints(dimensions, false, false, true);
      expect(points.length).toBeGreaterThan(0);
